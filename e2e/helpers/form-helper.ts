...
export class FormHelper {
  ...
  async createForm(title: string, description?: string): Promise<string> {
    ...
    await this.page.waitForSelector('[role="dialog"], .modal, .create-form-dialog', { timeout: 5000 });
    ...
    await this.page.click('button[type="submit"]:has-text("Create"), button:has-text("Create Form"):not(:disabled)');
  ...
  async addField(field: FormField): Promise<void> {
    ...
    await this.page.waitForSelector('.field-item:last-child, .block-item:last-child', { timeout: 5000 });
    
    // Configure field
    await this.page.fill('input[placeholder*="Question"], input[placeholder*="Title"], input[name="title"]:visible', field.title);
    
    if (field.description) {
      await this.page.fill('textarea[placeholder*="Description"], input[placeholder*="Description"], input[name="description"]:visible', field.description);
    }
    
    if (field.placeholder) {
      await this.page.fill('input[placeholder*="Placeholder"], input[name="placeholder"]:visible', field.placeholder);
    }
    
    // Set required if needed
    if (field.required !== undefined) {
      const requiredSwitch = await this.page.locator('input[type="checkbox"][name="required"], [role="switch"][aria-label*="Required"]').first();
    ...
    const saveButton = await this.page.locator('button:has-text("Save"):visible, button:has-text("Done"):visible').first();
  ...
  async publishForm(): Promise<string> {
    ...
    await expect(this.page.locator('text=published successfully, text=Form published')).toBeVisible({ timeout: 10000 });
    
    // Get public URL
    const publicUrlElement = await this.page.locator('[data-testid="public-url"], .public-url, input[readonly][value*="/f/"]').first();
    const publicUrl = await publicUrlElement.inputValue().catch(() => publicUrlElement.textContent());
  ...
  async fillAndSubmitForm(formUrl: string, data: Record<string, any>): Promise<void> {
    ...
    for (const [fieldName, value] of Object.entries(data)) {
      if (fieldName === 'file') {
        ...
        // Handle signature
        const canvas = await this.page.locator('canvas[data-signature], .signature-pad canvas').first();
        await this.drawSignature(canvas);
      } else {
        // Handle text inputs
        const input = await this.page.locator(`input[name="${fieldName}"], textarea[name="${fieldName}"], [aria-label*="${fieldName}" i]`).first();
  ...
  async configureWebhook(formId: string, webhookUrl: string): Promise<void> {
    ...
    const allEventsCheckbox = await this.page.locator('input[type="checkbox"][name="all_events"], [role="checkbox"][aria-label*="All events"]').first();
    ...
    // Wait for success
    await expect(this.page.locator('text=Webhook added, text=Webhook created')).toBeVisible({ timeout: 5000 });
  }
}