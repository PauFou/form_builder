...
describe('FormBuilderStore', () => {
      ...
      const mockForm: Form = {
        ...
        pages: [
          { 
            ...
            title: 'Page 1', 
            blocks: [
              { id: 'block-1', type: 'short_text', question: 'Original question' }
            ] 
          }
        ],
        createdAt: new Date(),
  ...
  describe('undo/redo', () => {
    it('should undo and redo changes', () => {
      ...
      // Undo
      store.undo();
      expect(store.form?.pages[0].blocks).toHaveLength(0);

      // Redo
      store.redo();
      expect(store.form?.pages[0].blocks).toHaveLength(1);
      expect(store.form?.pages[0].blocks[0].id).toBe('block-1');
    });
  });
});