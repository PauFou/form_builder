...
function detectLocale(): Locale {
  ...
  if (typeof window !== "undefined" && window.navigator) {
    ...
    const mappedLocale =
      browserLocaleMap[browserLang] || browserLocaleMap[browserLang.split("-")[0]];
    if (mappedLocale) {
      return mappedLocale;
    }
  }
...
export function I18nProvider({ children }: { children: ReactNode }) {
  ...
  const formatRelativeTime = (date: Date | string | number): string => {
    ...
    if (diffInSeconds < 60) {
      ...
      return hours === 1 ? t.time.hour_ago : t.time.hours_ago.replace("{count}", String(hours));
    } else if (diffInSeconds < 604800) {
      const days = Math.floor(diffInSeconds / 86400);
      return days === 1 ? t.time.day_ago : t.time.days_ago.replace("{count}", String(days));
    } else if (diffInSeconds < 2592000) {
      const weeks = Math.floor(diffInSeconds / 604800);
      return weeks === 1 ? t.time.week_ago : t.time.weeks_ago.replace("{count}", String(weeks));
    } else if (diffInSeconds < 31536000) {
      const months = Math.floor(diffInSeconds / 2592000);
      return months === 1 ? t.time.month_ago : t.time.months_ago.replace("{count}", String(months));
    } else {
      const years = Math.floor(diffInSeconds / 31536000);
      return years === 1 ? t.time.year_ago : t.time.years_ago.replace("{count}", String(years));
    }
  };
