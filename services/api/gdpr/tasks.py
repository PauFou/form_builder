...
def process_deletion_request(self, request_id: str, processor_id: str):
    ...
    try:
        ...
        if deletion_request.scope == 'all':
            ...
            deleted_counts = _delete_all_data(
            ...
            deleted_counts = _delete_form_data(
                deletion_request.requester_email,
            ...
            deleted_count = _delete_submissions(
...
def _delete_all_data(email: str, organization: Organization) -> Dict[str, int]:
...
def _delete_form_data(email: str, form) -> Dict[str, int]:
...
def _delete_submissions(submission_ids: List[str]) -> int:
    """Delete specific submissions"""
...
def process_export_request(self, request_id: str):
    ...
    try:
        ...
        if export_request.include_submissions:
            submissions_data = _collect_submissions(
        ...
        if export_request.include_partial_submissions:
            partials_data = _collect_partials(
                export_request.requester_email,
                export_request.organization
            )
        ...
        if export_request.include_consent_records:
            consent_data = _collect_consent_records(
                export_request.requester_email,
                export_request.organization
            )
        ...
        if export_request.include_audit_logs:
            audit_data = _collect_audit_logs(
                export_request.requester_email,
                export_request.organization
            )
        ...
        file_url, file_size = _generate_export_file(
            export_data,
...
def _collect_submissions(email: str, organization: Organization) -> List[Dict]:
...
def _collect_partials(email: str, organization: Organization) -> List[Dict]:
...
def _collect_consent_records(email: str, organization: Organization) -> List[Dict]:
...
def _collect_audit_logs(email: str, organization: Organization) -> List[Dict]:
...
def _generate_export_file(data: Dict, format: str, request_id: str) -> tuple:
    ...
    if format == 'json':
        ...
        file_content = _json_to_csv(data).encode('utf-8')
    elif format == 'parquet':
