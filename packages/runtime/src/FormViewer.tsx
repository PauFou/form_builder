...
export function FormViewer({ schema, config }: FormViewerProps) {
  ...
  if (currentStep >= schema.blocks.length) {
    return (
      <div style={{
        ...
        <div dangerouslySetInnerHTML={{ __html: schema.settings?.thankYouMessage || '<h2>Thank you!</h2>' }} />
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} style={{
      ...
      {/* Input field */}
      {renderField(currentBlock, formData[currentBlock.id], (value) => handleChange(currentBlock.id, value))}
