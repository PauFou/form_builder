...
describe("FormViewerWrapper", () => {
  ...
  it("switches between modes when clicking buttons", async () => {
    ...
    expect(screen.getByLabelText("Grid mode - show all questions")).toHaveAttribute("aria-pressed", "true");
    
    // Switch to one-question mode
    await user.click(screen.getByLabelText("One question at a time mode"));
    
    expect(screen.getByLabelText("One question at a time mode")).toHaveAttribute("aria-pressed", "true");
    expect(screen.getByLabelText("Grid mode - show all questions")).toHaveAttribute("aria-pressed", "false");
  });
