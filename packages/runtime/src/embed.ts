...
(function() {
  ...
  function loadForm(options: EmbedOptions) {
    const containerId = typeof options.container === 'string' 
      ? options.container 
    ...
    // Show loading state
    container.innerHTML = '<div style="text-align:center;padding:20px;color:#6b7280;">Loading form...</div>';
    ...
    fetch(configUrl)
      ...
      .catch(err => {
        container!.innerHTML = '<div style="text-align:center;padding:20px;color:#ef4444;">Failed to load form</div>';
  ...
  function autoInit() {
    ...
    forms.forEach(el => {
      ...
      const mode = el.getAttribute('data-forms-mode') as 'inline' | 'popup' | 'drawer' || 'inline';
      if (formId) {
        loadForm({
          formId,
